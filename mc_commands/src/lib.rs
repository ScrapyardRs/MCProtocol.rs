...
use mc_serializer::serde::{
    Contextual, Deserialize, Error, ProtocolVersion, Serialize, SerializerContext,
};
...
impl Serialize for Command {
    fn serialize<W: Write>(
        ...
        wrap_struct_context!(
            "command_flags",
            self.command_flags.serialize(writer, protocol_version)
        )?;
        wrap_struct_context!(
            "children",
            self.children.serialize(writer, protocol_version)
        )?;
            ...
            match self.redirect {
                None => {
                    return Err(Error::Generic(SerializerContext::new(
                        Self::context(),
                        "Found `None` redirect when one was specified.".to_string(),
                    )))
                }
                Some(redirect) => {
                    wrap_struct_context!("redirect", redirect.serialize(writer, protocol_version))?
                }
            }
        }
        match &self.node_stub {
            NodeStub::Argument {
                ...
                wrap_struct_context!(
                    "node_argument_argument",
                    argument.serialize(writer, protocol_version)
                )?;
                wrap_struct_context!(
                    "node_resource_location",
                    resource_location.serialize(writer, protocol_version)
                )?;
            }
            Literal(literal) => {
    ...
    fn size(&self, protocol_version: ProtocolVersion) -> mc_serializer::serde::Result<i32> {
        ...
        if self.command_flags & 8 != 0 {
            match self.redirect {
                ...
                Some(redirect) => {
                    size += wrap_struct_context!("redirect", redirect.size(protocol_version))?
                }
            }
        }
        match &self.node_stub {
            NodeStub::Argument {
                ...
                size += wrap_struct_context!(
                    "node_argument_argument",
                    argument.size(protocol_version)
                )?;
                size += wrap_struct_context!(
                    "node_resource_location",
                    resource_location.size(protocol_version)
                )?;
            }
            Literal(literal) => {
...
impl Deserialize for Command {
    fn deserialize<R: Read>(
        ...
        let flags = wrap_struct_context!("flags", i8::deserialize(reader, protocol_version))?;
        let children_size = VarInt::deserialize(reader, protocol_version)?;
        let mut children = Vec::new();
        for _ in 0i32..children_size.into() {
            let next = VarInt::deserialize(reader, protocol_version)?;
            children.push(next);
        }
        ...
        let node_stub = match flags & 3 {
            2 => {
                let id = wrap_struct_context!(
                    "argument_identifier",
                    String::deserialize(reader, protocol_version)
                )?;
                let argument = wrap_struct_context!(
                    "argument_argument",
                    Argument::deserialize(reader, protocol_version)
                )?;
            ...
            x => {
                return Err(mc_serializer::serde::Error::Generic(
                    SerializerContext::new(
                        Self::context(),
                        format!("Failed to recover command node of type {}", x),
                    ),
                ));
            }
        };
        ...
        Ok(Command {
            ...
            children: (children_size, children),
            redirect,
            node_stub,
        })
    }
