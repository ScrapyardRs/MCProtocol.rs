...
fn expand_mc_serde_struct(input: &DeriveInput, data_struct: &DataStruct) -> TokenStream {
    ...
    let ser = quote! {
        impl mc_serializer::serde::Serialize for #struct_ident {
            fn serialize<W: std::io::Write>(&self, writer: &mut W) -> mc_serializer::serde::SerdeResult<()> {
                #(#self_resolvers)*
                #serialize_stream
                Ok(())
            }

            fn size(&self) -> mc_serializer::serde::SerdeResult<i32> {
                let mut size = 0;
    ...
    quote! {
        ...
        impl mc_serializer::serde::Deserialize for #struct_ident {
            fn deserialize<R: std::io::Read>(reader: &mut R) -> mc_serializer::serde::SerdeResult<Self> {
                #deserialize_stream
                Ok(Self #make_stream)
            }
        }
...
fn expand_mc_serde_enum(input: &DeriveInput, data_enum: &DataEnum) -> TokenStream {
    ...
    for variant in &data_enum.variants {
        ...
        (quote! {
            #input_ident::#variant_ident #raw_make_stream => {
                mc_serializer::serde::Serialize::serialize(&#key, writer)?;
                #serialize_stream
                Ok(())
            }
        })
        ...
        (quote! {
            #input_ident::#variant_ident #raw_make_stream => {
                let mut size = 0;
                size += mc_serializer::serde::Serialize::size(&#key)?;
                #size_stream
                Ok(size)
            }
        })
    ...
    quote! {
        impl mc_serializer::serde::Serialize for #input_ident {
            fn serialize<W: std::io::Write>(&self, writer: &mut W) -> mc_serializer::serde::SerdeResult<()> {
                match self {
                    #serializer_stream
                }
        ...
        impl mc_serializer::serde::Deserialize for #input_ident {
            fn deserialize<R: std::io::Read>(reader: &mut R) -> mc_serializer::serde::SerdeResult<Self> {
                let key_value: #key_type = mc_serializer::serde::Deserialize::deserialize(reader)?;
                #deserializer_stream
                Err(mc_serializer::serde::Error::Generic(format!("Failed to understand key {:?} for {}", key_value, stringify!(#input_ident))))
            }
        }
    }
}
...
fn field_serializer_dynamic(fields: &Fields) -> TokenStream {
    ...
    for (index, field) in fields.into_iter().enumerate() {
        ...
        let der = quote!(mc_serializer::serde::Serialize::serialize(#field_ident, writer)?;);
    ...
    match fields {
        Fields::Named(named_fields) => {
            ...
            for field in &named_fields.named {
                ...
                let der = quote!(let #fake_field_ident = anyhow::Context::context(mc_serializer::serde::Deserialize::deserialize(reader), format!("Failure to write property: {}", stringify!(#fake_field_ident)))?;);
        ...
        Fields::Unnamed(unnamed_fields) => {
            ...
            for (index, _) in unnamed_fields.unnamed.iter().enumerate() {
                ...
                let der = quote!(let #field_ident = mc_serializer::serde::Deserialize::deserialize(reader)?;);
                field_deserializer_stmts.push(der);
            }
...
fn field_sizer_dynamic(fields: &Fields) -> TokenStream {
    ...
    for (index, field) in fields.into_iter().enumerate() {
        let field_ident = mapped_ident(PREFIX, index as i32, &field.ident.as_ref());
        let der = quote!(size += mc_serializer::serde::Serialize::size(#field_ident)?;);
        serializer_stmts.push(der);
    }
