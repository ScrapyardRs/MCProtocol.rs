...
use mc_registry::client_bound::play::{ChangeDifficulty, DeclareCommands, DeclareRecipes, EntityEvent, JoinGame, LevelChunkWithLight, LightUpdate, Ping, PlayerAbilities, PlayerInfo, PlayerPosition, PluginMessage, SetCarriedItem, SetChunkCacheCenter, SetDefaultSpawnPosition, SystemChat, UpdateRecipes, UpdateTags, WorldBorder};
    ...
    loop {
        ...
        .await?
        {
            ...
            Some(unhandled) => {
                ...
                if unhandled.packet_id == VarInt::from(98) { // system chat
                    continue;
                }
                if unhandled.packet_id == VarInt::from(76) { // set chunk cache radius
                    continue;
                }
                if unhandled.packet_id == VarInt::from(90) { // set simulation distance
                    continue;
                }
                if unhandled.packet_id == VarInt::from(0) { // add entity
                    continue;
                }
                if unhandled.packet_id == VarInt::from(80) { // set entity data
                    continue;
                }
                if unhandled.packet_id == VarInt::from(104) { // update attributes
                    continue;
                }
                if unhandled.packet_id == VarInt::from(63) { // rotate head
                    continue;
                }
                if unhandled.packet_id == VarInt::from(83) { // set equipment
                    continue;
                }
                if unhandled.packet_id == VarInt::from(92) { // set time
                    continue;
                }
                if unhandled.packet_id == VarInt::from(29) { // Game event
                    continue;
                }
                if unhandled.packet_id == VarInt::from(66) { // Server data
                    continue;
                }
                if unhandled.packet_id == VarInt::from(17) { // Container set content
                    continue;
                }
                if unhandled.packet_id == VarInt::from(85) { // set health
                    continue;
                }
